...
namespace Microsoft.MixedReality.OpenXR
{
        ...
        : OpenXRFeature, IOpenXRContext, ISubsystemPlugin where TPlugin : OpenXRFeaturePlugin<TPlugin>
    {
        ...
        protected override void OnEnable()
        {
            ...
            // This work would ideally happen in ScriptableObject::Awake, but this is not accessible here without OpenXRFeature exposing a new virtual method.
            base.OnEnable();
            if (m_feature == null)
            {
                // This method must be called from the main thread, which OnEnable is guaranteed to be called on.
                m_feature = OpenXRSettings.Instance.GetFeature<TPlugin>();
            }

            PluginEnvironmentSubsystem.InitializePlugin();
        }

        internal static TPlugin Feature
        {
            get
            {
                if (!PluginEnvironmentSubsystem.Initialized)
                {
                    Debug.LogError($"[OpenXRFeaturePlugin] Feature plugins have not been loaded.");
                }
                return (TPlugin)m_feature;
            }
        }
