...
def get_features_mean(X_features):
    """
    Calculate means of all time series features (36 features in P36 dataset).
...
# 均值插补
# 首先计算每个样本的有效时间长度，即去除缺失值后的时间点数量；然后，仅对有效时间长度内的缺失值进行插补，用所有训练样本的均值来替代缺失值
...
# 前向插补
# 首先计算每个样本的有效时间长度，即去除缺失值后的时间点数量；然后，仅对有效时间长度内的缺失值进行插补，用前一个非缺失值来替代当前缺失值
...
# 三次样条插补
# 首先计算每个样本的有效时间长度，即去除缺失值后的时间点数量；然后，用三次样条插值的方法根据非缺失值计算出插补值
...
# 总结：这三种插补方法均是基于每个样本的有效时间长度进行插补，以确保只对实际观测到的时间点内的缺失值进行处理，保证数据的一致性和合理性
