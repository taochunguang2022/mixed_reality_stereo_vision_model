...
for f in range(n_sensors):
    ...
    # 训练支持向量机模型
    svc = SVC(probability=True).fit(feature_train, label_train)

    # 评估模型性能
    svc_acc = svc.score(feature_test, label_test)
    svc_acc_train = svc.score(feature_train, label_train)

    svc_result = svc.predict(feature_test)

    if dataset == 'P36' or dataset == 'P12':
      auc_score = roc_auc_score(label_test, svc.predict_proba(feature_test)[:, 1])
