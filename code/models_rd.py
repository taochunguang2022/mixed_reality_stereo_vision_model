...
class Raindrop_v2(nn.Module):
    ...
    def __init__(self, d_inp=36, d_model=64, nhead=4, nhid=128, nlayers=2, dropout=0.3, max_len=215, d_static=2,
    ...
    def forward(self, src, static, times, lengths):
        ...
        static = Pstatic: [128, 2]: this one doesn't matter; static features
        ...
        missing_mask = src[:, :, self.d_inp:int(2*self.d_inp)]#后36个是mask   src表示时序特征  (215,128,72)
        src = src[:, :, :int(src.shape[2]/2)]#src表示时序特征
        n_sensor = self.d_inp

        src = torch.repeat_interleave(src, self.d_ob, dim=-1)#沿指定维度复制多少倍原来每个传感器有1个值，现在复制2倍，一摸一样
        h = F.relu(src*self.R_u)  # h是实际的输入 对数值的embedding 原来是1个值，现在是2个值
        pe = self.pos_encoder(times) #对时间做位置编码
        if static is not None:
            emb = self.emb(static)  # 对2个静态特征做embedding 2变成36

        h = self.dropout(h)

        mask = torch.arange(maxlen)[None, :] >= (lengths.cpu()[:, None])  # 每个记录都有个有效长度
        ...
        if step1 == False:
            ...
            batch_size = src.shape[1]  # 128条记录，每个记录是一个图，单独构建每个图，batch是图与图无关
            n_step = src.shape[0]
            ...
            for unit in range(0, batch_size): #对每个记录遍历，每个记录是个图
                stepdata = x[:, unit, :] # x是个时序特征数据 取当前记录的时序特征数据
                p_t = pe[:, unit, :] # 当前记录时刻的位置编码
                ...
                stepdata, attentionweights = self.ob_propagation(stepdata, p_t=p_t, edge_index=edge_index,
                                                                 ...
                                                                 use_beta=use_beta, edge_attr=None,
        ...
        if static is not None:
            output = torch.cat([output, emb], dim=1)#时序+静态特征
