"""Here is an initial version of the proposed Raindrop model.  """
wandb = False
...
parser.add_argument('--dataset', type=str, default='P36', choices=['P12', 'P36']) #
...
parser.add_argument('--splittype', type=str, default='random', choices=['random', 'gender'], help='all use')
parser.add_argument('--reverse', default=False, help='if True,use female for tarining; if False, use male for training') #
...
parser.add_argument('--predictive_label', type=str, default='fatigue', choices=['fatigue'],
                    help='use this  P12, P36 dataset (fatigue)')
...
for missing_ratio in missing_ratios:
    ...
    for k in range(n_splits):
        ...
        if dataset == 'P36':
            if subset == True:
                split_path = '/splits/phy36_split_subset' + str(split_idx) + '.npy'
            else:
                split_path = '/splits/phy36_split' + str(split_idx) + '.npy'
        elif dataset == 'P12':
        ...
        global_structure = torch.ones(d_inp, d_inp)   # 定义全局结构 36*36 点与点之间全连接 指定边
        ...
        if missing_ratio > 0:
            ...
            if feature_removal_level == 'sample':
                ...
                density_score_indices = np.load('./ohtermodels/saved/IG_density_scores_' + dataset + '.npy', allow_pickle=True)[:, 0]
        ...
        for m in range(n_runs):
            ...
            if dataset == 'P12'or dataset == 'P36':
                model = Raindrop_v2(d_inp, d_model, nhead, nhid, nlayers, dropout, max_len,
                                    ...
                                    sensor_wise_mask=sensor_wise_mask)   # 定义模型的网络结构
            ...
            expanded_idx_1 = np.concatenate([idx_1,idx_1], axis=0)     # 把不均衡的样本复制2份
            ...
            for epoch in range(num_epochs):
                ...
                if epoch == 0 or epoch % 1 == 0:
                    with torch.no_grad():
                        ...
                        if dataset == 'P12' or dataset == 'P3' or dataset == 'P6' or dataset == 'P10' or dataset == 'P11'or  dataset == 'P36':
            ...
            #   plot_confusion_matrix(ytest, ypred, split_idx)
